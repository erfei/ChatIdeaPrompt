...
function inputbasekj(mycontent){
    ...
    // 获取 <p> 标签内的文本内容，忽略 placeholder
    // 获取所有 <p> 标签，拼接为多行文本
    const paragraphs = $$('p', inputElement);
    let currentInputContent = '';

    if (paragraphs.length > 0) {
        const lines = Array.from(paragraphs)
            .filter(p => !p.classList.contains('is-empty'))
            .map(p => p.textContent || '');
        
        // 至少有一行非空内容才拼接
        if (lines.some(line => line.trim() !== '')) {
            currentInputContent = lines.join('\n');
        }
    }
    ...
    try {
       ...
       const paragraphs = $$('p', inputElement);
        if (paragraphs.length > 0) {
            ...
            // 剩余行用 execCommand 补充（此时已在 catch 块，降级处理）
        } else {
            inputElement.textContent = replacedText;
        }
        inputElement.dispatchEvent(new Event('input', { bubbles: true }));
        inputElement.dispatchEvent(new Event('change', { bubbles: true }));
        console.warn("⚠️ 使用了备用输入方案");
